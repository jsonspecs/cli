...
function loadSamples(dir, pipelineId = null) { return listSampleFiles(dir).flatMap(({ file, full }) => { try { const body = JSON.parse(fs.readFileSync(full, 'utf8')); return !pipelineId || body?.pipelineId === pipelineId ? [{ name: file.replace(/\.json$/, ''), file, body }] : []; } catch (_) { return []; } }).sort((a, b) => compareUtf16(a.name, b.name)); }
