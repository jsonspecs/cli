...
function startStudio(project, options = {}) {
  ...
  const server = app.listen(port, host, () => console.log(terminal.studio(`listening on http://${host}:${port}`, 'ok')));
...
function compileIntoCtx(engine, operatorMeta, project, ctx, verbose = false) {
  ...
  if (verbose) console.log(ctx.terminal.studio(`compiled ${artifacts.length} artifacts from ${project.rulesDir}`, 'ok'));
}
...
function startHotReload(project, ctx) {
  ...
  function reload(changedFile) {
    ...
    console.log(`\n${ctx.terminal.studio(`changed: ${rel}`, 'reload')}`);
    console.log(ctx.terminal.studio('recompiling...', 'info'));
    try {
      ...
      ctx.emit('reload');
      console.log(ctx.terminal.studio('OK', 'ok'));
    } catch (err) {
      console.error(ctx.terminal.studio('COMPILATION ERROR - keeping previous version', 'error'));
  ...
  console.log(ctx.terminal.studio(`watching ${project.rulesDir}`, 'info'));
  return watchers;
}
